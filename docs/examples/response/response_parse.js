...
export function parse_sis_xml(sisxml) {
  ...
  let resp_dict_group = hard_resp[0].getElementsByTagNameNS(
    SIS_NS,
    "ResponseDictGroup",
  );
  let stages = [];
  let resp_dict = new Map();
  let rd_array = Array.from(
    resp_dict_group[0].getElementsByTagNameNS(SIS_NS, "ResponseDict"),
  );
  rd_array.forEach((rdict) => {
    ...
    if (resp_el.localName === "FIR") {
      ...
      let s_array = Array.from(
        resp_el.getElementsByTagNameNS(SIS_NS, "FilterStage"),
      );
      s_array.forEach((fs) => {
        let seqNum = fs.getElementsByTagNameNS(SIS_NS, "SequenceNumber")[0]
          .textContent;
        let sis_filter_ref = fs.getElementsByTagNameNS(SIS_NS, "Filter")[0];
        let fname = sis_filter_ref.getElementsByTagNameNS(SIS_NS, "Name")[0]
          .textContent;
        ...
        if (gainXml.length > 0) {
          ...
          throw new Error(
            "Did not find Gain in stage number " + seqNum + " " + fname,
          );
        }
