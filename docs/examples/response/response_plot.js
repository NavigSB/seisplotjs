...
document
  .querySelector("#extstationxml_load")
...
function load_fdsn() {
  ...
  stationQuery
    ...
    .then((networkList) => {
      ...
      if (!firstChan) {
        document.querySelector("div.stageplot").innerHTML =
          `<p>No channels found</p>`;
      }
      if (!firstChan.response) {
        console.log(`No response: ${firstChan.channelCode}`);
        document.querySelector("div.stageplot").innerHTML =
          `<p>No response: ${firstChan.channelCode}</p>`;
      }
      const chanChooser = document.querySelector("sp-channel-list");
      chanChooser.addEventListener("change", () => {
        chanChooser
          .selectedChannels()
          .forEach((c) => process_stages(c.response.stages));
      });
      chanChooser.setChannels(
        Array.from(sp.stationxml.allChannels(networkList)),
      );
...
function load_ext(input_selector) {
  ...
  const timeoutSec = 10;
  sp.util
    .doFetchWithTimeout(url, fetchInit, timeoutSec * 1000)
    .then((response) => {
    ...
    .then((xml) => {
      ...
      if (top.localName === "Response") {
        ...
        top.localName === "FDSNStationXML" &&
        xml.querySelector("HardwareResponse")
      ) {
        ...
        if (!firstChan.response.stages) {
          console.log(
            `No stages: ${firstChan.channelCode}  ${firstChan.response}`,
          );
        }
        const chanChooser = document.querySelector("sp-channel-list");
        chanChooser.setChannels(
...
function process_stages(stages) {
  ...
  stages.forEach((stage, idx) => {
    if (stage.filter && stage.decimation !== null) {
      ...
      if (first_sps === null) {
        first_sps = stage.decimation.inputSampleRate;
      }
    }
  });
  stages.forEach((stage, idx) => {
    let div = document
      .querySelector("div.stageplot")
      .appendChild(document.createElement("div"));
    let details = div.appendChild(document.createElement("details"));
    details.appendChild(document.createElement("summary")).textContent =
      `Stage: ${idx + 1}`;
    ...
    if (
      ...
      let impulseResponse = sp.fft.FFTResult.createFromPackedFreq(
        sp.fft.calcDFT(coeff.reverse()),
        coeff.length,
      ...
      stage.filter &&
      stage.filter instanceof sp.stationxml.PolesZeros &&
      stage.filter.pzTransferFunctionType === "LAPLACE (RADIANS/SECOND)"
    ) {
      const numPoints = pad_size;
      let gamma = 0;
      let sacPoleZero = sp.transfer.convertPoleZeroToSacStyle(
        stage.filter,
        1,
        1,
        gamma,
      ...
      let freqAmp = new sp.spectraplot.FreqAmp(
        freqs,
        sacPoleZero.calcForDisplay(freqs),
      );
      ...
      let div = document
        .querySelector("div.stageplot")
        .appendChild(document.createElement("div"));
      let filter_class = stage.filter
        ? stage.filter.__proto__.constructor.name
        : "missing";
      let len_msg = "";
      if (!stage.filter) {
...
function calc_stage_coeff(stage) {
  if (stage.filter instanceof sp.stationxml.FIR) {
    ...
    if (fir.symmetry === "NONE") {
      ...
      longCoeff[fir.numerator.length - 1] =
        fir.numerator[fir.numerator.length - 1];
...
function plot_from_packed_freq(div, stage, idx, impulseResponseList) {
  ...
  const fftAmpPlot = new sp.spectraplot.SpectraPlot(
    impulseResponseList,
    plotConfig,
  );
  ...
  if (phaseCB.checked) {
    ...
    const fftPhasePlot = new sp.spectraplot.SpectraPlot(
      impulseResponseList,
      plotConfig,
    );
...
function plot_impulse(div, stage, idx, coeff) {
  ...
  let impulseSeis = sp.seismogram.Seismogram.fromContiguousData(
    coeff.reverse(),
    sampleRate,
    start,
  );
  ...
  let seisData =
    sp.seismogram.SeismogramDisplayData.fromSeismogram(impulseSeis);
...
function stage_details(stage, idx) {
  ...
  if (stage.filter && stage.filter instanceof sp.stationxml.PolesZeros) {
    ...
    stage.filter.poles.forEach((p, idx) => {
      out += `${idx}: ${p.real()} + ${p.imag()} i\n`;
    });
    out += `Zeros (${stage.filter.zeros.length}):\n`;
    stage.filter.zeros.forEach((p, idx) => {
      out += `${idx}: ${p.real()} + ${p.imag()} i\n`;
    });
  } else if (
    stage.filter &&
    ...
    stage.filter.numerator.forEach((p, idx) => {
      out += `${idx}: ${p}\n`;
    });
    out += `Denominator (${stage.filter.denominator.length}):\n`;
    stage.filter.denominator.forEach((p, idx) => {
      out += `${idx}: ${p}\n`;
    });
  } else if (stage.filter && stage.filter instanceof sp.stationxml.FIR) {
