...
let createFIR = function () {
  ...
  document.querySelector("h3.coefficients").textContent =
    `FIR Coefficients: N: ${N} => ${2 * N + 1}  OmegaP: ${OmegaP} Wp: ${Wp}  OmegsS: ${OmegaS} Ws: ${Ws}`;
  ...
  let asText = "";
  firLp.getCoefficients().forEach((d, i, a) => {
    asText += coeffDispFun(d, i, a);
  });
  ...
  const firPlot = fftfir_div.appendChild(
    new sp.spectraplot.SpectraPlot([impulseResponse], plotConfig),
  );

  firPlot.logfreq = doLogLog;
  firPlot.draw();
  const firPhasePlot = fftfir_div.appendChild(
    new sp.spectraplot.SpectraPlot([impulseResponse], plotConfig),
  );
  ...
  graph.seismographConfig.linkedTimeScale.duration =
    seisData.timeRange.toDuration();
...
function fftForwardArray(dataArray, sampRate = 1) {
  ...
  let tryFFT = sp.fft.FFTResult.createFromPackedFreq(
    packetFreq,
    dataArray.length,
    sampRate,
  );
